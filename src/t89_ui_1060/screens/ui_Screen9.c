...
// boss-to-tip distance is exactly NEEDLE_LEN on the 800x480 design canvas. On
// this 1024x600 variant ui_scale.h adds a 1.25x runtime scale on top, which is
// what keeps the needle proportional to the scaled-up dial. Rotation and scale
// are one combined transform in LVGL, so this does not cost a second pass.
...
void ui_Screen9_screen_init(void)
{
    ...
    // The pivot stays in SOURCE image pixels — ui_scale.h deliberately leaves
    // lv_image_set_pivot() alone, because the 1.25x it applies to the image is
    // a transform ABOUT that pivot, not a resample of the source.
    lv_image_set_pivot(dial_pointer, POINTER_PIVOT_X, POINTER_PIVOT_Y);
    // So the position has to mix spaces by hand: the dial centre is a scaled
    // dial coordinate, the pivot offset is unscaled source pixels. Written with
    // the function name parenthesised to bypass the ui_scale.h shim, which
    // would otherwise scale the pivot term too and leave the boss ~9 px off the
    // dial centre.
    (lv_obj_set_pos)(dial_pointer, ui_s(DIAL_R) - POINTER_PIVOT_X,
                                   ui_s(DIAL_R) - POINTER_PIVOT_Y);

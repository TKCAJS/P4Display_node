...
static void needle_point_at(float rpm)
{
    ...
    // The point list is the one piece of geometry ui_scale.h cannot reach:
    // lv_line_set_points() keeps the caller's array rather than copying it, so
    // it can't be shimmed through a temporary. Scale the offsets here instead.
    // Each is the difference of two SCALED absolutes, not the scale of a
    // difference, so the tip lands on exactly ui_s(tip_x) once lv_obj_set_pos
    // (which the shim does scale) has placed the line at ui_s(min_x).
    needle_points[0].x = ui_s(tail_x) - ui_s(min_x);
    needle_points[0].y = ui_s(tail_y) - ui_s(min_y);

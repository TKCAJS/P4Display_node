...
/**
 * @brief LOCAL PATCH (P4Display_node): PPA-rotate a LOGICAL (landscape) sub-rect
 *        into its mapped position in the BACK native framebuffer, growing the
 *        same dirty row band board_p4_flush_region() feeds. This is the LVGL
 *        PARTIAL-render flush path: only invalidated areas are rotated/written,
 *        instead of present_rotated()'s full 800x480 frame per refresh.
 *        Call board_p4_present() once on the refresh's last flush, as with
 *        board_p4_flush_region().
 * @param lx,ly,lw,lh  dirty rect in LOGICAL landscape coords (800x480 space).
 * @param src          lw*lh contiguous RGB565 pixels (row-major, stride == lw).
 */

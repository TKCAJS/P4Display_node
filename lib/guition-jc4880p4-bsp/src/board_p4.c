...
void board_p4_flush_region_rotated(int lx, int ly, int lw, int lh, const uint16_t *src)
{
    // LOCAL PATCH (P4Display_node): partial-render companion to
    // board_p4_present_rotated() — PPA-rotate ONE logical dirty rect into its
    // mapped native position in the back FB, and grow the dirty row band so
    // board_p4_present() only re-syncs those rows. Same rotation angle as the
    // full-frame path; the rect mapping must be its spatial equivalent.
    if (!s_framebuffer || !s_ppa_srm || !src || lw <= 0 || lh <= 0) return;

    // Clip in LOGICAL space (stride == lw, so clipping left/top moves src).
    if (lx < 0) { lw += lx; src -= lx; lx = 0; }
    if (ly < 0) { lh += ly; src -= (size_t)ly * (lw > 0 ? lw : 0); ly = 0; }
    if (lx + lw > BOARD_P4_LCD_V_RES /*800*/) lw = BOARD_P4_LCD_V_RES - lx;
    if (ly + lh > BOARD_P4_LCD_H_RES /*480*/) lh = BOARD_P4_LCD_H_RES - ly;
    if (lw <= 0 || lh <= 0) return;

    // Map the logical rect to native coords. A 90/270 rotation swaps the rect's
    // dimensions: native w = lh, native h = lw.
#if BOARD_P4_PPA_ROTATE == PPA_SRM_ROTATION_ANGLE_270
    const int nx0 = BOARD_P4_LCD_H_RES - ly - lh;   // 480 - ly - lh
    const int ny0 = lx;
#elif BOARD_P4_PPA_ROTATE == PPA_SRM_ROTATION_ANGLE_90
    const int nx0 = ly;
    const int ny0 = BOARD_P4_LCD_V_RES - lx - lw;   // 800 - lx - lw
#else
#error "board_p4_flush_region_rotated: only 90/270 rotation supported"
#endif
    const int nw = lh;
    const int nh = lw;
    ...
    ppa_srm_oper_config_t srm = {
        ...
        .out = {
            ...
            .buffer_size    = (uint32_t)((size_t)BOARD_P4_LCD_H_RES * BOARD_P4_LCD_V_RES * sizeof(uint16_t)),
            .pic_w          = BOARD_P4_LCD_H_RES,
            .pic_h          = BOARD_P4_LCD_V_RES,
            .block_offset_x = (uint32_t)nx0,
            .block_offset_y = (uint32_t)ny0,
            .srm_cm         = PPA_SRM_COLOR_MODE_RGB565,
        },
        .rotation_angle = BOARD_P4_PPA_ROTATE,
        .scale_x        = 1.0f,
        .scale_y        = 1.0f,
        .mirror_x       = false,
        .mirror_y       = false,
    ...
    if (s_dirty_y1 == 0) {
        ...
        if (ny0 < s_dirty_y0)      s_dirty_y0 = ny0;
        if (ny0 + nh > s_dirty_y1) s_dirty_y1 = ny0 + nh;
    }
}
